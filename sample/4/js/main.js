(function() {
  ...
  root.MY_CLASS.graphicView = (function(_super) {
    ...
    function graphicView() {
      this._setGraParts = __bind(this._setGraParts, this);
      this.update = __bind(this.update, this);
      this.resize = __bind(this.resize, this);
      this.dispose2 = __bind(this.dispose2, this);
      ...
      this._topCanvas;
      this._outputCanvas;
      this._imgData[0] = new root._LIBS.image(root.MY.conf.PATH_IMG + "photo.jpg", 450, 450);
      this._parts = [];
    }

    graphicView.prototype.addStage = function() {
      var _this = this;
      this._area = new root._LIBS.display(this.id() + "_area", {
        update: false,
        resize: false
      });
      this.addChild(this._area);
      this._imgData[0].onMakeData = function() {
        _this._bgCanvas = new root._LIBS.display(_this.id() + "_bgCanvas", {
          update: false,
          resize: false
        });
        _this.addChild(_this._bgCanvas);
        _this._bgCanvas.size(_this._imgData[0].width, _this._imgData[0].height);
        _this._bgCanvas.canvas();
        _this._bgCanvas.ctx().drawImage(_this._imgData[0].data(), 0, 0);
        _this._bgCanvas.elm().hide();
        return _this._setGraParts();
      };
      this._imgData[0].makeData();
      return this.setMousePos();
    };
    ...
    graphicView.prototype.resize = function() {
      ...
      w = root.MY.main.ws.w;
      h = root.MY.main.ws.h;
      this.xy(~~(w * 0.5 - this._imgData[0].width * 0.5), ~~(h * 0.5 - this._imgData[0].height * 0.5));
      this._area.size(w, h);
      return this._area.xy(-this.x(), -this.y());
    };

    graphicView.prototype.update = function() {
      var h, i, m, val, valX, valY, w, _i, _len, _ref, _results;
      w = root.MY.main.ws.w;
      h = root.MY.main.ws.h;
      m = this.mouse();
      valX = (w * 0.5 - m.x) * 0.0001;
      valY = (h * 0.5 - m.y) * 0.0001;
      _ref = this._parts;
      _results = [];
      for (i = _i = 0, _len = _ref.length; _i < _len; i = ++_i) {
        val = _ref[i];
        _results.push(val.updateCanvas(valX, valY));
      }
      return _results;
    };

    graphicView.prototype._setGraParts = function() {
      var parts,
        _this = this;
      parts = new root.MY_CLASS.graphicParts(this.id() + "_parts", this._bgCanvas);
      this.addChild(parts);
  ...
  root.MY_CLASS.graphicParts = (function(_super) {
    ...
    function graphicParts(elm, bgCanvas) {
      this.modeReset = __bind(this.modeReset, this);
      this._setPol = __bind(this._setPol, this);
      this._draw = __bind(this._draw, this);
      this.updateCanvas = __bind(this.updateCanvas, this);
      this._initPol = __bind(this._initPol, this);
      this.dispose2 = __bind(this.dispose2, this);
    ...
    graphicParts.prototype.addStage = function() {
      ...
      u = root.MY.util;
      this._bm = new bm();
      this._topCanvas = new root._LIBS.display(this.id() + "_topCanvas", {
        resize: false,
        update: false
      });
      this.addChild(this._topCanvas);
      this._topCanvas.size(this._bgCanvas.width(), this._bgCanvas.height());
      this._topCanvas.canvas();
      this._topCanvas.elm().hide();
      this._outputCanvas = new root._LIBS.display(this.id() + "_outputCanvas", {
        resize: false,
        update: false
      });
      this.addChild(this._outputCanvas);
      this._outputCanvas.size(this._bgCanvas.width(), this._bgCanvas.height());
      this._outputCanvas.canvas();
      return this.modeReset();
    };

    graphicParts.prototype.dispose2 = function() {};

    graphicParts.prototype._initPol = function() {
      var i, val, _i, _j, _len, _len1, _ref, _ref1;
      if (this._triangles != null) {
        _ref = this._triangles;
        for (i = _i = 0, _len = _ref.length; _i < _len; i = ++_i) {
          val = _ref[i];
          if (val != null) {
            val.dispose();
            this._triangles[i] = null;
          }
        }
        this._triangles = [];
      }
    ...
    graphicParts.prototype.updateCanvas = function(valX, valY) {
      var angleX, angleY, i, val, _i, _len, _ref;
      this._speed.x += (valX - this._speed.x) * 0.1;
      this._speed.y += (valY - this._speed.y) * 0.1;
      angleX = this._speed.x;
      angleY = this._speed.y;
      _ref = this._points;
      for (i = _i = 0, _len = _ref.length; _i < _len; i = ++_i) {
        val = _ref[i];
        if (val != null) {
      ...
      this._draw();
      return this._bm.apply(this._bgCanvas.canvasId(), this._topCanvas.canvasId(), this._outputCanvas.canvasId(), this._bmType);
    };

    graphicParts.prototype._draw = function() {
      var ctx, i, u, val, _i, _len, _ref, _results;
      u = root.MY.util;
      u.sort(this._triangles, "depth", true);
      ctx = this._topCanvas.ctx();
      ctx.clearRect(0, 0, this._topCanvas.width(), this._topCanvas.height());
      _ref = this._triangles;
      _results = [];
      for (i = _i = 0, _len = _ref.length; _i < _len; i = ++_i) {
        val = _ref[i];
        _results.push(val.draw(ctx));
      }
      return _results;
    };

    graphicParts.prototype._setPol = function() {
      var angle, color16, i, index, light, numFaces, radius, rotX, rotY, u, val, xpos, ypos, _i, _len, _ref;
      u = root.MY.util;
      numFaces = u.random(15, 30);
      radius = u.random(150, 250);
      color16 = parseInt(String(this._color).replace("#", ""), 16);
      light = new root._LIBS.light(0, 0, -radius * 20, 1);
      index = 0;
      i = 0;
      while (i < numFaces) {
        ...
        this._points[index] = new root._LIBS.point3d(xpos, ypos, -radius * 0.5);
        this._points[index + 1] = new root._LIBS.point3d(xpos, ypos, radius * 0.5);
        index += 2;
        i++;
      }
      rotX = u.radian(u.range(180));
      rotY = u.radian(u.range(180));
      _ref = this._points;
      for (i = _i = 0, _len = _ref.length; _i < _len; i = ++_i) {
        val = _ref[i];
        val.setCenter(0, 0, radius);
        val.setVanishingPoint(this._topCanvas.width() * 0.5, this._topCanvas.height() * 0.5);
        val.rotateX(rotX);
        val.rotateY(rotY);
      }
      index = 0;
      i = 0;
      while (i < numFaces - 1) {
        this._triangles[index] = new root._LIBS.triangle(this._points[index], this._points[index + 3], this._points[index + 1], color16, light);
        this._triangles[index + 1] = new root._LIBS.triangle(this._points[index], this._points[index + 2], this._points[index + 3], color16, light);
        index += 2;
        i++;
      }
      this._triangles[index] = new root._LIBS.triangle(this._points[index], this._points[1], this._points[index + 1], color16, light);
      this._triangles[index + 1] = new root._LIBS.triangle(this._points[index], this._points[0], this._points[1], color16, light);
      return this._draw();
    };

    graphicParts.prototype.modeReset = function() {
      var arr;
      this._color = root.MY.util.getRandomColor();
      arr = [this._bm.SCREEN, this._bm.LINEARDODGE, this._bm.LIGHTEN];
      this._bmType = arr[root.MY.util.random(0, arr.length - 1)];
      this._initPol();
      return this._setPol();
    };
